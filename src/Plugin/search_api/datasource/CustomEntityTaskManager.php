 ...
 */
class CustomEntityTaskManager implements EventSubscriberInterface {

  /**
   * The Search API task type used by this service for "insert items" tasks.
   */
  const INSERT_ITEMS_TASK_TYPE = 'search_api.custom_tables_datasource.trackItemsInserted';

  /**
   * The Search API task type used by this service for "delete items" tasks.
   */
  const DELETE_ITEMS_TASK_TYPE = 'search_api.custom_tables_datasource.trackItemsDeleted';
  ...
  /**
   * Constructs a CustomEntityTaskManager object.
   *
  ...
  public function processEvent(TaskEvent $event, $event_name) {
    ...
    // The complete event name prefix in front of the method name is 52
    // characters long: "search_api.task.search_api.custom_tables_datasource.".
    $method = substr($event_name, 52);
    ...
    if ($index->isValidDatasource($datasource_id)) {
      /** @var \Drupal\search_api\Plugin\search_api\datasource\CustomEntity $datasource */
